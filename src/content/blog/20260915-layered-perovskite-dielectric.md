...
老師提供的兩份結構為 rMBrThMA₂PbI₄ 與 rMClThMA₂PbI₄。
...
Br full model 使用 4 × 6 × 6 K 點所得的零頻率電子介電實部張量為：

| 分量 | X | Y | Z |
|---|---:|---:|---:|
| X | 4.763763 | 0.001077 | 0.278936 |
| Y | 0.001077 | 4.355388 | 0.062089 |
| Z | 0.278936 | 0.062089 | 4.579660 |

其跡平均值為：

**ε̄ = Tr(ε) / 3 = 4.5663**

由 3 × 4 × 4 加密至 4 × 6 × 6 後，三個對角分量的最大變化約 0.22%，跡平均值變化約 0.14%。這表示**零頻率電子介電實部**對這次 K 點加密已相當穩定。
...
兩篇方法參考論文都不是單純以頻率趨近零的介電值比較有機 barrier 與無機 well，而是依各材料的實驗發光波長，取得對應能量下的介電響應。

波長與光子能量可用下式換算：

**E_PL（eV）= 1239.84 / λ_PL（nm）**

目前的工作判讀是：

- 主要整理 Re[ε(E_PL)]，討論該發光能量下的介電響應。
- 同時保留 Im[ε(E_PL)]，檢查吸收與損耗是否顯著。
- Re[ε(0)] 可作為額外的電子介電參考值，但不能代替論文在 PL 能量的比較。
...
目前仍缺少的是 rMBrThMA₂PbI₄ 與 rMClThMA₂PbI₄ 自身的實驗 PL 峰值或其原始文獻來源。
...
- Br 與 Cl 的晶軸方向不同，K 點網格與張量方向都必須依各自晶胞檢查。
